...
import os # import os for file the path handling
from dotenv import dotenv_values # import dotenv for reading environment variables from a .env file
import time

# Load environment variables from a .env file
env_vars = dotenv_values(".env")
AssistantVoice = env_vars.get("AssistantVoice") # Get the AssistantVoice from the environmental variables

# Asynchronous function to convert text to an audio file
async def TextToAudioFile(text)-> None:
    file_path = os.path.join("Data", "speech.mp3") # Define the path where the speech file will be saved4
    
    if os.path.exists(file_path): # Check if the file already exists
        os.remove(file_path) # If it exists , remove it to avoid overwritting errors
        
        # create the communicate Text-To-Specch (TTS) Asynchronously
...
def TTS(Text, func=lambda r=None: True):
        try:
            ...
            except Exception as e: # Handle any exception during clean up
...
def TextToSpeech(Text, func=lambda r=None: True):
    ...
    # If the text is verey long (more than 4 sentences and 250 characters ), add a response message
